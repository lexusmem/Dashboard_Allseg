...
arquivo_excel = r'C:\Users\alex.sousa\Documents\Dados_Sinistros\Cópia de Precificacao - Copia.xlsx'
...
def carregar_e_processar_dados(caminho_arquivo):
    ...
    try:
        ...
        aba_apolice_endosso = pd.read_excel(caminho_arquivo, sheet_name='apolice_endosso')
        ...
        dados_adicionais.rename(columns={'cd_apolice': 'N° Apólice'}, inplace=True)

        # Merge dos dados de prêmio com os dados adicionais
        premio_com_dados = pd.merge(soma_por_apolice, dados_adicionais, on='N° Apólice', how='left')

        # Carrega a aba 'sinistro'
        aba_sinistro = pd.read_excel(caminho_arquivo, sheet_name='sinistro')
        aba_sinistro['Total Sinistro'] = aba_sinistro['vl_sinistro_total'] + aba_sinistro['vl_despesa_total'] + aba_sinistro['vl_honorario_total'] - aba_sinistro['vl_salvado_total']
        
        # Soma dos sinistros por apólice:
        soma_sinistro_por_apolice = aba_sinistro.groupby('cd_apolice')['Total Sinistro'].sum().reset_index()
        soma_sinistro_por_apolice.rename(columns={'cd_apolice': 'N° Apólice', 'Total Sinistro':'Soma Sinistro Por Apolice'}, inplace=True)
        ...
        st.error(f"Erro: O arquivo '{caminho_arquivo}' não foi encontrado. Por favor, verifique o caminho.")
...
def carregar_e_processar_dados_sinistro(caminho_arquivo):
    ...
    try:
        ...
        dados_de_sinistro.rename(columns={'cd_apolice': 'N° Apólice'}, inplace=True)
...
def formatar_valor_br(valor):
    ...
    valor_br_format = valor_us_format.replace(",", "X").replace(".", ",").replace("X", ".")
...
dados_exibicao.loc[:, 'Soma Prêmio Pago por Apolice'] = dados_exibicao['Soma Prêmio Pago por Apolice'].map(formatar_valor_br)
dados_exibicao.loc[:, 'Soma Sinistro Por Apolice'] = dados_exibicao['Soma Sinistro Por Apolice'].map(formatar_valor_br)
...
if apolices_selecionadas_filtro_apolice:
    dados_filtrados_filtro_apolice = dados_filtrados_filtro_apolice[dados_filtrados_filtro_apolice['N° Apólice'] == apolices_selecionadas_filtro_apolice]
...
df_para_filtro_apolice['Soma Prêmio Pago por Apolice'] = df_para_filtro_apolice['Soma Prêmio Pago por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
df_para_filtro_apolice['Soma Sinistro Por Apolice'] = df_para_filtro_apolice['Soma Sinistro Por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
...
percentual_sinistro_total_filtro_apolice = (total_sinistro_filtro_apolice / total_premio_filtro_apolice) if total_premio_filtro_apolice != 0 else 0

col_apl_1, col_apl_2, col_apl_3 = st.columns(3)

with col_apl_1:
    st.metric(label="Total Prêmio Pago", value=f"R$ {formatar_valor_br(total_premio_filtro_apolice)}")
with col_apl_2:
    st.metric(label="Total Sinistro", value=f"R$ {formatar_valor_br(total_sinistro_filtro_apolice)}")
with col_apl_3:
    st.metric(label="% Sinistro Total", value=f"{percentual_sinistro_total_filtro_apolice:.2%}")
...
representante = list(dados_filtrados_filtro_apolice['nm_representante'].unique())
utilização = list(dados_filtrados_filtro_apolice['nm_auto_utilizacao'].unique())


with col_seg_1:
    st.markdown("<p style='margin-bottom: 0;'>Segurado</p>", unsafe_allow_html=True)
    st.markdown(f"<h6 style='margin-top: 0; margin-bottom: 0.2rem;'>{segurado[0].title()}</h6>", unsafe_allow_html=True)
with col_cor_2:
    st.markdown("<p style='margin-bottom: 0;'>Corretor</p>", unsafe_allow_html=True)
    st.markdown(f"<h6 style='margin-top: 0; margin-bottom: 0.2rem;'>{corretor[0].title()}</h6>", unsafe_allow_html=True)
with col_rep_3:
    st.markdown("<p style='margin-bottom: 0;'>Representante</p>", unsafe_allow_html=True)
    st.markdown(f"<h6 style='margin-top: 0; margin-bottom: 0.2rem;'>{representante[0].title()}</h6>", unsafe_allow_html=True)
with col_util_4:
    st.markdown("<p style='margin-bottom: 0;'>Utilização</p>", unsafe_allow_html=True)
    st.markdown(f"<h6 style='margin-top: 0; margin-bottom: 0.2rem;'>{utilização[0].title()}</h6>", unsafe_allow_html=True)
...
if apolices_selecionadas_filtro_apolice:
    dados_apolices_segurado = dados_apolices_segurado[dados_apolices_segurado['nm_estipulante'] == segurado[0]]


df_pr_sin_segurado = dados_apolices_segurado.copy()

df_pr_sin_segurado['Soma Prêmio Pago por Apolice'] = df_pr_sin_segurado['Soma Prêmio Pago por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
df_pr_sin_segurado['Soma Sinistro Por Apolice'] = df_pr_sin_segurado['Soma Sinistro Por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
...
total_sinistralidade_segurado = (total_sinistro_segurado / total_pr_segurado) if total_pr_segurado != 0 else 0
total_apolice_segurado = df_pr_sin_segurado['N° Apólice'].nunique()




seg_apl_1, seg_apl_2, seg_apl_3, seg_apl_4 = st.columns(4)
...
with seg_apl_3:
    st.metric(label="% Sinistro Total", value=f"{total_sinistralidade_segurado:.2%}")
with seg_apl_4:
    st.metric(label='Qtd. Apolices Emitidas', value=total_apolice_segurado)
...
representantes_unicos = sorted(dados_exibicao['nm_representante'].astype(str).unique())
...
if representantes_selecionados:
    dados_filtrados_rep = dados_filtrados_rep[dados_filtrados_rep['nm_representante'].astype(str).isin(representantes_selecionados)]
...
corretores_unicos = sorted(dados_filtrados_rep['nm_corretor'].astype(str).unique())
...
if corretores_selecionados:
    dados_filtrados_corr = dados_filtrados_corr[dados_filtrados_corr['nm_corretor'].astype(str).isin(corretores_selecionados)]
...
segurados_unicos = sorted(dados_filtrados_corr['nm_estipulante'].astype(str).unique())
...
if segurados_selecionados:
    dados_filtrados_segurado = dados_filtrados_segurado[dados_filtrados_segurado['nm_estipulante'].astype(str).isin(segurados_selecionados)]
...
df_para_soma['Soma Prêmio Pago por Apolice'] = df_para_soma['Soma Prêmio Pago por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
df_para_soma['Soma Sinistro Por Apolice'] = df_para_soma['Soma Sinistro Por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
...
with col3:
    st.metric(label="% Sinistro Total", value=f"{percentual_sinistro_total:.2%}")
...
if not resultado_final_filtrado.empty:
    ...
    df_para_groupby['Soma Prêmio Pago por Apolice'] = df_para_groupby['Soma Prêmio Pago por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
    df_para_groupby['Soma Sinistro Por Apolice'] = df_para_groupby['Soma Sinistro Por Apolice'].str.replace('.', '').str.replace(',', '.').astype(float)
    ...
    groupby_utilizacao.rename(columns={'nm_auto_utilizacao': 'Utilização'}, inplace=True)

    # Ordene o DataFrame pelo 'Total_Premio' em ordem decrescente
    groupby_utilizacao = groupby_utilizacao.sort_values(by='Total_Premio', ascending=False)
